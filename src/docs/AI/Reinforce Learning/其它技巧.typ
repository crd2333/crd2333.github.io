...
- 之前讲的 MA 都是基于 CTDE 的假设，即智能体在学习的过程中可以获取全局信息，但是在执行的过程中是分布式的。但是在一些情况下，智能体在学习和执行的过程中都是分布式的，这就是完全去中心化学习
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-09-45-22.png", width: 80%)
...
- 思路：智能体不同步更新，每次只有一个智能体进行策略更新
  - IQL 没有收敛保证，MA2QL 有
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-09-59-53.png", width: 80%)
- MA2QL：多智能体轮流策略迭代
  - 有收敛保证（纳什均衡），即收敛但未必全局最优
  - 这种轮流迭代的方法无论是基于策略还是基于价值函数，最终都会收敛到纳什均衡
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-10-09-46.png", width: 80%)
  - 轮流更新与单步策略提升需要太久收敛
...
#grid(
  ...
  - 一般分为以下几个模块（右图）：
    ...
    - 通信模块
    - 数据与模型聚合模块
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-10-25-09.png", width: 80%)
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-10-26-01.png", width: 80%)
  ],
  [
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-10-22-08.png", width: 80%)
  - 模型聚合的基本方式（左图）：
...
- 并行PPO(DPPO)
  ...
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-10-46-48.png", width: 80%)
  - 哪些流程可以并行：
  ...
  #fig("/public/assets/AI/AI_RL/img-2024-07-12-10-48-58.png", width: 80%)
- DPPO 存在的问题：
...
== 模仿学习
#q[来自西瓜书《机器学习 —— 周志华》]
- 强化学习任务中多步决策的搜索空间巨大，基于累积奖励来学习很多步之前的合适决策非常困难
- 缓解方法：直接模仿人类专家的状态-动作对来学习策略
  - 相当于告诉机器在什么状态下应该选择什么动作
  - 引入了监督信息来学习策略
- 通过直接模仿学习
  - 直接回归出一个更好的策略，然后作为强化学习更好的初始值，大大加速收敛速度
  - 步骤：
    + 利用专家的决策轨迹，构造数据集 $D$：状态作为特征，动作作为标记
    + 利用数据集 $D$，使用分类/回归算法即可学得策略
    + 将学得的策略作为强化学习的初始策略
- 通过逆强化学习
  - 动机：强化学习任务中，设计合理的符合应用场景的奖励函数往往相当困难。是否能从人类专家提供的范例数据中反推出奖励函数？
  - 基本思想：欲使机器做出与范例一致的行为等价于在某个奖励函数的环境中求解最优策略，该最优策略所产生轨迹与范例数据一致。换言之，希望寻找某种奖励函数使得范例数据是最优的，然后即可使用这个奖励函数来训练策略
  - 西瓜书中
    - 把奖励函数假设表达为状态特征的线性函数 $R(x) = w^T x$
    - 然后一顿猛如虎的数学推导，得出了若能对所有策略计算即可导出 $w^*$ 的闭式解
    - 不过显然难以获得所有策略，因此采用随机策略开始，迭代式地求解更好的奖励函数，即*迭代式逆强化学习算法*
    #fig("/public/assets/AI/AI_RL/2025-01-04-19-51-00.png", width: 50%)
