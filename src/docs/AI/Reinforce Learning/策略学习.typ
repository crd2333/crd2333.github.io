...
- 回顾
  #fig("/public/assets/AI/AI_RL/img-2024-07-05-09-36-52.png", width: 50%)
  #fig("/public/assets/AI/AI_RL/img-2024-07-05-09-38-37.png", width: 50%)
- 策略参数化（例如 DQN）
  - 将策略参数化为$pi_th (a|s)$，其中 $theta$ 是参数
  - 对于确定性策略 $a = pi_th (s)$，对于随机策略 $pi_th (a|s) = P(a|s,th)$（将可见的已知状态泛化到未知的状态上）
...
- 通常做法是参数化策略 $pi_theta$，并利用无梯度或基于梯度的优化方法对参数进行更新
  ...
  - 对比之前的表格型策略
    + 采取某个动作的概率的计算方式不同：状态 $s$ 上采取动作 $a$ 的概率，一个是直接*查表*，一个是*计算* $pi_theta (a|s)$
    + 策略的更新方式不同：一个是直接*修改表格对应条目*，一个是*更新参数* $theta$
    + 最优策略的定义不同
      - 表格型策略称 $pi^*$ 是最优的，若它能最大化每个状态对应的值函数
        $ pi^* (s) = argmax_(a in cA) Q_pi (s, a), s in cS $
      - 参数化策略：称一个策略 $pi^\#$ 是最优的，若它能最大化一个给定的标量指标
        $ pi^\# = argmax_th J(pi(s)), pi in oPi $
        - $oPi$ 为策略容许集，取决于参数 $th$ 的取值范围
        - 若 $pi^* in oPi$，则有 $J(pi^\#)=J(pi^*)$；否则，$pi^*$ 比 $pi^\#$ 更优
- 基本思想：
...
- 目标函数
  #fig("/public/assets/AI/AI_RL/img-2024-07-05-09-59-33.png", width: 50%)
- 状态分布$d(s)$
  - 策略无关的状态分布
    - 在这类情况下，目标函数关于参数的梯度通常更好算
    -  一个简单的做法是取 $d(s)$ 为均匀分布，即每个状态都有相同的权重 $1\/|S|$
    - 另一种做法是把权重集中分配给一部分状态集合
      - 例如，在一些任务中，一个回合只从状态 $s_0$ 开始，那么可以设置为：$d(s_0)=1,d(s!=s_0)=0$
  - 策略相关的状态分布
    - 在这种情况下，通常选用稳态状态分布
    - $d(s)$ 是稳态状态分布：若对一个状态转移 $s->a->s'$，满足：
      #mitex(`d(s^{\prime})=\sum_{s\in\mathcal{S}}\sum_{a\in\mathcal{A}}p(s^{\prime}|s,a)\cdot\pi_{\theta}(a|s)\cdot d(s)`)
- 如果采用最大化平均轨迹回报目标函数$ max_theta J(theta)=max_theta EE_(tau wave p_theta (tau)) [sum_t r(s_t,a_t)] $
  ...
  #fig("/public/assets/AI/AI_RL/img-2024-07-05-10-18-35.png", width: 50%)
- 另外两种目标函数和策略梯度
  #fig("/public/assets/AI/AI_RL/img-2024-07-05-10-19-19.png", width: 50%)
- 使用不同的策略梯度，以及不同的近似方法，我们可以得到各种各样的基于策略梯度的强化学习算法，如 REINFORCE、DDPG、PPO 等

=== REINFORCE 算法
- 对于随机策略 $pi_theta (a,s) = P(a|s, theta)$
  - 直觉上我们应该：降低带来低价值/奖励的动作出现的概率；提高带来高价值/奖励的动作出现的概率
- 上一章中我们推导出了策略梯度（最大化平均轨迹回报目标函数），在实践中，我们可以用蒙特卡洛方法进行估计
  #mitex(`\nabla_{\theta}J(\theta)=\ \frac{1}{N}\sum_{n=1}^{N}\sum_{t=1}^{T^{n}}R(\tau^{n})\nabla_{\theta}l o g\,\pi_{\theta}(a_{t}^{n}|s_{t}^{n})`)
- 据此，我们可以得到 REINFORCE 算法
#algo(title: "REINFORCE 算法")[
...
#fig("/public/assets/AI/AI_RL/img-2024-07-05-15-16-33.png", width: 60%)
- 思考强化学习与分类问题对比
  + 都是输入状态，输出要采取的行为
  + 分类问题（监督学习）：假设有带标签的训练数据，随后利用极大似然法进行优化
  + 强化学习：没有标签，只能通过试错的方式与环境交互获取奖励，以替代监督信息进行训练
- 一个问题——*训练可能存在偏差*
  #fig("/public/assets/AI/AI_RL/img-2024-07-05-10-41-17.png", width: 60%)
...
#fig("/public/assets/AI/AI_RL/img-2024-07-05-15-17-48.png", width: 70%)
- Advantage Actor-Critc (A2C) 算法
  - 思想：通过减去一个基线值来标准化评论家的打分
    - 降低较差动作概率，提高较优动作概率
    - 进一步降低方差
  - 优势函数：$A^pi (s,a)=Q^pi (s,a)-V^pi (s_t)$，改变 critic，于是原本 AC 算法的策略梯度
    #mitex(`\nabla_{\theta}J(\theta)\approx\frac{1}{N}\sum_{i=1}^{N}\left(\sum_{t=1}^{T}V_{\theta}\,l o g\,\pi_{\theta}(a_{t}^{i}|s_{t}^{i})\bar{Q}^{\pi}(s_{i}^{i},\alpha_{t}^{i})\right)`)
    变为
  ...
  - 状态值估计$hat(V)^pi$能否与策略 $pi_theta$ 共用网络？
    #fig("/public/assets/AI/AI_RL/img-2024-07-05-11-15-17.png", width: 70%)
  ...
  - 批量更新（没听懂）
    ...
    - 解决方案：获得一个批次的数据后再进行更新，分为同步和异步两种方法
      #fig("/public/assets/AI/AI_RL/img-2024-07-05-11-22-12.png", width: 70%)
    - 批量更新再改进
      #fig("/public/assets/AI/AI_RL/img-2024-07-05-11-22-32.png", width: 70%)

- Asynchronous Advantage Actor-Critc(A3C) 算法
  - 与A2C一样使用优势函数
  - 异步的Actor-Critic方法能够充分利用多核 CPU 资源采样环境的经验数据，利用GPU资源异步地更新网络，这有效提升了强化学习算法的训练效率
...
- 目标函数等价形式写成
  #mitex(`J(\theta)~=~\mathbb{E}_{S}\left[\mathbb{E}_{A\cdots\pi(\cdot|S;\theta_{\mathrm{now}})}\left[\frac{\pi(A\mid S;\,\theta)}{\pi(A\mid S;\,\theta_{\mathrm{now}})}~\cdot\,{\cal Q}_{\pi}(S,A)\right]\right]`)
