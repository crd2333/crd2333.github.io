...
- SMPL，详细见 #link("http://crd2333.github.io/note/Reading/Sparse%20view%20Reconstruction/SMPL")[SMPL 笔记]
  ...
  - SMPL 影响力非常大，已经成为人体重建任务广泛应用的一个基础先验(prior)或模板(template)，它主要是针对裸体人体的 shape（高矮胖瘦）和 pose（动作姿势）进行*参数化(parametric)*建模（不是神经网络那个参数化的意思！），因此可以给人体重建任务最关键 geometry and appearance 中的前者提供先验。因为 shape 参数是通过 PCA 降维出的对人体表示影响最大的几个维度，因此 SMPL 也被称为基于 *statistical* 的模型
...
- 而 CV, CG 学界为了重建相对精确的人体，宽泛的意义上一般有这几种 setting
  ...
  + *基于多视角图片或单目视频的方法*
    - 通过多视角带位姿视图，弥补单张图片信息的不足，从而提高重建的精度。这类方法往往需要多个相机，或者通过多帧视频来获取多视角信息
    - 视频数据的获取比之扫描数据相对容易得多了。另外，视频中蕴含的时间上的连续性和语义性的先验可以帮助重建算法对抗单目的歧义性
    - 在这个 setting 下，NeRF 和 3DGS 等表示方法已经可以大展身手
  + *基于单张图片重建的方法*
    - 相比起前两种方法，单目数据的获取成本最低，做成了之后应用范围也是最广的
    ...
    - 但它也是最具挑战性的，因为单目数据的信息量有限，很难直接从单张图片中恢复出人体的三维形状
      ...
      - 或者引入一些先验知识，比如用 SMPL 引入 geometry prior，或者用图片生成模型(Diffusion Model)引入 appearance(texture) prior
...
- 不过如前所述，人体重建有几何、纹理颜色、高精度要求、训练数据稀缺等的复杂性
  - 这就导致这一领域的方法往往比较复杂，需要引入各种先验知识，包含很多 handcrafted 特征和模块（还没到能用大量数据进行 end-to-end 训练的地步），各种方法的混合使用非常普遍（比一般三维重建 topic 更加 hybrid）
  - 个人觉得再用 Implicit 和 Explicit 来区分感觉不那么现实了，因此下面选择梳理一遍从 PIFu, PIFuHD, PaMIR 到 ICON, ECON, SHERF 再到 GTA, SIFU 这些工作的发展脉络
    - [ ] todo: 之后再梳理，顺便加上后面看的论文
  // - 受 SIFU 的启发，从引入先验多少的角度，这些工作可以粗略地抽象为以下三个步骤
  //   #fig("/public/assets/reading/human/2024-11-19-17-41-12.png",width:60%)
  //   - PIFu 没有引入任何 geometry prior，它单纯使用 image encoder 得到的 2D feature，对 3D 空间的任意点进行投影得到对应 feature，连带着深度送入 MLP 去预测 occupancy + color（(b) 阶段只用了最简单的投影）
  //   - PIFuHD 跟 PIFu 差不多，也没有引入 geometry prior，但它在 (a) 多了个从 input image 猜 normal map 的过程，从而进行 coarse-to-fine 的重建
  //   - PaMIR 开始引入 SMPL prior，它的 (a), (b) 跟 PIFu 差不多，但在 (c) 阶段用 input image 估计出 SMPL body mesh，然后用 3D global encoder 提取 volume feature，把投影得到的 2D feature 和 volume feature 一起送入 MLP
  //   - ICON 在 (a), (c) 引入了 SMPL prior，在 SMPL 的帮助下预测 normal map，normal map 和 SMPL 的迭代优化或许可以看作是 (b) 中引入了一定的 geometry prior，最后把 SMPL-guided SDF, SMPL body normal（(c) 的 prior 引入）和 normal map 送入 MLP
  //   - ECON
