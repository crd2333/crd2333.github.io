...
- 光圈(Aperture)与光圈数(F-number)
  通过放大/缩小光圈来控制图像的亮度；$f/D$，$f$ 是焦距，$D$ 是光圈直径，光圈数越大，光圈越小，进光量越少。*光圈数*基本上也就等同于透镜的大小（直径）
- Lens Defocus
  当 $f$ 和 $i$ 固定时，只有一个面(深度)在成像平面上是清楚的，所以我们需要对焦。弥散圆(Blur circle diameter):$b=D/i' |i'-i|$，下图是生活中常见的一个例子
...
- 引入齐次坐标，将投影表示为线性变换
$
mat(f,0,0,0;0,f,0,0;0,0,1,0) vec(x,y,z,1) = vec(f x,f y,z) = vec(f x/z, f y/z, 1)
$
