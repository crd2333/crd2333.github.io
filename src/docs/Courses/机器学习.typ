...
- 评估方法
  ...
  + 留出法(Hold-out)
    - 在保证*数据分布的一致性*下，直接将数据集划分为训练和测试集，常用比例为 $2:1 wave 4:1$
  + 交叉验证法(Cross Validation)
    - 在保证*数据分布的一致性*下，将数据集划分为 $k$ 个大小相等的子集，每次取 $k-1$ 个子集作为训练集，剩下的一个子集作为测试集（做 $k$ 次训练）
  + 自助法(Bootstrapping)
    - 从数据集 $D$ 中有放回地抽取 $N$ 个样本，作为训练集 $D'$；剩下的样本作为测试集
    - 一个样本始终不被采样到的概率是 $lim_(N -> infty) (1-1/N)^N = 1/3 approx 0.368$
- 性能度量
...
- 间隔与支持向量：支持向量机的"最大间隔”思想
  - 输入 $T={(x1,y1),dots, (xN,yN)}, x_i in RR^n, y_i in {-1,+1}$
  - 构造并求解约束最优化问题 #h(1fr)
    $
    &"原始形式" ~~~ max_(w,b) ga ~~~ s.t. ~ y_i (w/norm(w) dot x_i + b /norm(w)) >= ga, ~~ &i=1,2,dots,N \
    &"优化形式" ~~~ max_(w,b) 1/2 norm(w)^2 ~~~ s.t. ~ y_i (w dot x_i + b) >= 1, ~~ &i=1,2,dots,N
    $
  - 输出最大间隔分离超平面、分离决策函数、支持向量
    $
    w^* dot x + b^* = 0 \
    f(x) = sign(w^* dot x + b^*) \
    H_1, H_2: w^* dot x + b^* = 1, w^* dot x + b^* = -1
    $
- 对偶问题：对偶问题及其解的稀疏性
  - 先构造拉格朗日函数 $L(w,b,al)$，极小极大问题 $min_(w,b) max_al L(w,b,al)$ 转化为极大极小问题 $max_al min_(w,b) L(w,b,al)$ #h(1fr)
    $
    min_al ~ &1/2 sumiN sumjN al_i al_j y_i y_j (x_i dot x_j) - sumiN al_i \
    s.t. ~ &sumiN al_i y_i = 0 \
    &al_i >= 0, i=1,2,dots,N
    $
  - 求解得到最优解 $al^*$，进而选择 $al^*$ 的一个正分量 $al_j^*$，计算
    $ w^* = sumiN al_i^* y_i x_i,\ b^* = y_j - sumiN al_i^* y_i (x_i dot x_j) $
  - 太寄吧难算了，考试的时候直接画图从结果倒推吧
- 核函数：通过向高维空间映射解决线性不可分的问题
- 软间隔与正则化：引入"软间隔"缓解特征空间中线性不可分的问题
  - 对每个样本点引入松弛变量 $xi_i >= 0$
  - 目标函数和约束条件变为 #h(1fr)
    $
    min_(w,b,xi) ~ 1/2 norm(w)^2 + C sumiN xi_i \
    y_i (w dot x_i + b) >= 1 - xi_i, ~ i=1,2,dots,N \
...
- 性能度量
  ...
  - *内部指标(internal index)*：直接考察聚类结果，无参考模型，如 DB 指数，Dunn 指数
    ...
    - Dunn 指数对每个簇 $i$，考察簇内最大距离大且跟簇 $i$ 最小距离小，对所有这样的 $i$ 取最小值
    $
    ...
    "Dunn指数" ~ &"DI" = min_(1<=i<=k) min_(j!=i) frac(d_min (C_i, C_j), max_(1<=l<=k) "diam"(C_l))
    $
- 距离计算
  - 距离度量需满足的基本性质：非负性、同一性、对称性、直递性（三角不等式）
  - 常用距离函数：闵可夫斯基距离 #h(1fr)
...
- 低维嵌入
  - 多维缩放方法(Multiple Dimensional Scaling, MDS)：旨在寻找一个低维子空间，使得*距离*和样本原有距离近似不变
  - 主要思路：内积保距，从距离矩阵到内积矩阵，然后寻找低维子空间尽量保持样本内积不变。方法就是*SVD 分解*
- PCA
  - 步骤：给定矩阵 $X^((0)) = [~~]_(m times n)$（$m$ 个维度为 $n$ 的样本）
  ...
  - 核化线性降维 KPCA，引入非线性性
    - 这里的核函数跟 SVM 那边的是一个意思，主要思路就是把数据从样本空间映射到高维特征空间，在高维空间中的线性操作对应于原始空间中的非线性操作
    - 相当于是先把 $X$ 升维到更好分解的高维空间 $Z$，然后再做 PCA 降维到低维空间 $Y$
- 流形学习（参考 #link("https://zhuanlan.zhihu.com/p/40214106")[知乎文章]，比西瓜书讲得好多了）
  - 什么是流形？流形(manifold)是几何中的一个概念，它是高维空间中的几何结构，即空间中的点构成的集合。可以简单的将流形理解成二维空间的曲线、三维空间的曲面在更高维空间的推广
  - 什么是流形学习？流形学习(manifold learning)假设数据在高维空间的分布位于某一更低维的流形上，基于这个假设来进行数据的分析。对于降维，要保证降维之后的数据同样满足与高维空间流形有关的几何约束关系。除此之外，流形学习还可以用实现聚类，分类以及回归算法
  - 等距映射(Isometric Mapping, Isomap)
    - 使用了微分几何中测地线的思想，希望数据在向低维空间映射之后能够保持流形上的*测地线距离*。直观打个比方就是将 3D 的地球仪投影到 2D 的平面地图上，国家之间的*距离信息得以保留*
    - 计算任意两个样本之间的测地距离，然后根据这个距离构造距离矩阵。最后通过距离矩阵求解优化问题完成数据的降维，降维之后的数据保留了原始数据点之间的距离信息
  - 局部线性嵌入(Locally Linear Embedding, LLE)
    - 核心思想是每个样本点都可以由与它相邻的多个点的线性组合（体现了局部线性，且将变动限制在局部）来近似重构，这相当于用分段的线性面片近似代替复杂的几何形状，样本投影到低维空间之后要保持这种线性重构关系，即有相同的重构系数
    - 首先为每个样本找到近邻小标集合，计算出高维空间中的*重构系数*(close-form)，用同样的系数在低维空间中求解样本的低维投影
- 度量学习
