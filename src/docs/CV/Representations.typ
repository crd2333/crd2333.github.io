...
- *点云*
  ...
  - 点云是三维空间中点的无序集合，可以将其视为 3D 形状曲面的*离散化样本*。点云具有*无序性*、*点之间相对孤立*和*变换不变性*
  - 在点云上可以存储三维坐标 $x,y,z$、RGB、分类值、强度值、时间等等
  - CV 这边的很多传统方法都是基于点云，例如可以由 SfM 直接输出，很容易获得。点云表示灵活而高效，且多样性非常高，同一 3D 形状可以由不同的点云表示，但这有时也是缺点
  - 点云的不规则性使得它们相对较难用现有用于规则网格数据的神经网络进行处理（经典工作比如 PointNet）。另外，点云表示的信息量有限（本质是三维几何形状的低分辨率重采样，只能提供片面的几何信息）
- *三角面片*
  - 也叫三角网格、多边形网格（多边形可以被拆分为三角形），是 CG 中非常经典的表示方法，被广泛应用于建模、动画、渲染。另外，由于工业界经典而成熟的*图形渲染管线*是基于三角形的，所以很多其它 3D 场景表示到最后还是要求转换为三角网格
  - 三角面片上可以存储三角形的*顶点坐标*、*法向量*、*纹理坐标*、*颜色*等信息
  - 三角面片具有*顶点无序性*、*方向性*、*拓扑性*。与体素相比，三角网格仅对场景表面建模，更紧凑且占用内存少；与点云相比，三角网格提供了模拟点关系的*曲面点的连通性*。三角网格的建模满足*有边界的流形 (manifold with boundary)*，易于编辑和处理（经典工作比如 pixel2mesh）
  - 但是，由于 mesh 复杂的拓扑结构，基于 mesh 的渲染往往不可微，现在有一些近似方案但可能比较慢。在 differentiability 方面不如简单的 voxel 或后来的 3DGS，以及后面的隐式神经表示
- *深度图*
...
- 隐式表示相对显式表示的
  - 优点
    + *表示不再与空间分辨率相耦合*：这直接得益于隐式表示的连续性。一个直接推论是，隐式表示理论上具有“无限分辨率”，使超分辨率成为可能
    + *表征能力（有时）更强*：在神经网络的加持下，隐式表示可以拟合更复杂的特征和场景，通常比显式表示更加强大。但这也不绝对 (e.g. 3DGS)
    + *可泛化性*：跨神经隐式表示的泛化等同于学习函数空间上的先验，通过神经网络权重的预训练实现 —— 这通常被称为*元学习*
    + *相对易于学习*：与显式表示相比，隐式表示将场景抽象成函数、场，这天然更适合神经网络去拟合，易于与各种网络结构结合。神经隐式表征可以*灵活地融合到*可微分的基于学习的管线中
    + *空间需求一般相对较小*：只需存储神经网络参数（某种程度上可以看作是把显式方法所想要表示的内容*压缩*到了网络参数中）
  - 缺点
    + *需要后处理*：图形渲染管线是基于显式 triangle mesh 的，除此之外的其它显式方法或隐式方法要么用自己的一套渲染方案（比如 3DGS 的泼溅 splatting，NeRF 的体渲染 volume rendering），要么就探索如何转化为 triangle mesh
    + *过度平滑和伪影*：过度平滑，即图像放大后难以保持锐利和纹理，视觉上变得平滑，本质上是高频信息丢失；伪影，即 Artifacts，包括模糊、马赛克效应或者不自然纹理等现象
...
- 另外翻译一个来自 #link("https://github.com/vsitzmann/awesome-implicit-representations")[awesome-implicit-representations] 的观点
  #q[
...
- 常用的隐式神经表示，如 SDF, Occupancy Field, NeRF，这些无论是 function 还是 field 实际上都是一种映射关系：
  ...
  + *Occupancy*：空间中的一个点是否被物体占用，通常以 $0.5$ 为标准，大于 $0.5$ 倾向于认为点被曲面占用（在内部），小于 $0.5$ 倾向于没有被占用（在外部），$0.5$ 认为点在曲面上（即 $F(p)=0.5$ 表示一个曲面）。用神经网络去近似这个函数，相当于一个二分类任务，得到的*分类决策边界*等价于曲面描述
...
- 其实很多时候，显式和隐式的表示不是那么非黑即白，我在分类的时候也进行了一些斟酌
  ...
  - NeRF 有五花八门的基于 *baking* 思想把特征*固化*在显式表示的工作，以及融合了显式表示的 hybrid 工作
...
- 传统的三维重建弊端在于
  ...
  - 传统三维重建每个步骤的算法都较为复杂，不够端到端，不够简洁，不够鲁棒（但话又说回来，一味追求端到端也不好，因为这整个过程比如 rendering 是有物理含义在里面的，某些模块使用人工设计去加先验和 inductive biase 是有道理的）
...
- 然后再看相对近代和比较重要的 #link("http://crd2333.github.io/note/Reading/3D%20Representations/NeRF")[NeRF]
  ...
  - NeRF 究竟好在哪里呢？我个人参考一些资料提出以下几点
    ...
    + 重建这种从低维恢复高维信息的过程是具有多解性的，传统 SDF, mesh 等的表达相对来说比较确定性（即使有解，它也很难准确地去逼近）；而 NeRF 这个五维全光辐射场的概念以及 MLP 的强大拟合能力就让它优化的自由度变得非常高（但其实这个空间过大也不是好事，所以后续也有一些工作去增加约束和正则化），视角依赖性的考量也让高光的建模成为可能
