...
// == Rendering Pipeline
// 这里把几种渲染管线都梳理一遍，不止老师上课讲的那几种。这里少了 GPU-Driven Rendering Pipeline 的内容，放在最后一节课里。
...
//     + Quad Overdraw
//       - 无论是 Early-Z / Z-PrePass 还是 Deferred Rendering，都无法避免 Quad Overdraw 的问题，这对网格非常多且碎小的物体尤为严重。Quad Overdraw 的问题来自 GPU 硬件的处理：GPU 以 $2 times 2$ 的最小粒度进行像素处理（以便能够通过 uv 计算出 mipmap 等级），即使只有其中一个像素需要着色，也要将整个 $2 times 2$ 的 block 调度为活跃线程并执行片元着色
//       - 在最坏情形下，$4$ 个像素分属于 $4$ 个三角形，Forward 为每个像素运行材质采样和光照计算 $4$ 次；Deferred 为每个像素运行材质采样 $4$ 次、光照计算 $1$ 次；而 Visibility 的材质采样和光照计算为每个像素均只运行 $1$ 次，因为可以在重计算 barycentrics (screen space) 时算出 mip-level
...
= GPU 驱动的几何管线 Nanite
先讲了一堆 GPU-Driven Rendering Pipeline 的内容，尤其是《刺客信条大革命》的做法。
