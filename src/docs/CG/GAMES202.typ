...
- Shadow Map 背后的数学基础（RTR 的积分近似式，非常重要，后续也会继续用到）:
...
Shadow from environment lighting。前面没有考虑 visibility 如果要考虑的话就变得更为困难，总结为*多光源问题* (many-light problem) 和*难以采样*。同时也无法用积分拆分近似来处理，因为考虑 visibility 项后，support 小和变化 BRDF 两个条件都无法满足。工业界对此也没有好的办法，一般是摆烂只用一个主要光源做环境光照。未来的终极解决方案，或许是 Real-time Ray Tracing (RTRT)。下面会介绍一种可以非常准确得到环境光阴影但有一定代价的算法 Precomputed Radiance Transfer (PRT)
...
- Screen Space Ambient Occlusion (SSAO)
  ...
  - 如果是在 3D space，visibility 只需要往四周采样即可，但在 screen space，我们只能*在 Z-Buffer 中采样*！
    ...
    - 进一步，可以用 g-buffer 来存法向信息（延迟渲染普及之后），来实现半球上的 SSAO，真正把 $cos th$ 加权纳入考虑 ($->$ HBAO)
- Screen Space Directional Occlusion (SSDO)
  ...
  - 总之，SSDO 的光照细节会更好；并且它具有一切使用 Screen Space 算法的弊端，如 false occlusion、信息丢失（且由于无法像 SSAO 那样考虑来自远处的间接光，信息丢失更严重）
- Screen Space Reflection (SSR)
  - SSR 实际上不止反射，它几乎是把 Ray Tracing 搬到了屏幕空间里，因此可以处理 specular, glossy, diffuse 等情况（无非是反射光线数量和方向分布的问题，还可以对采样算法进行一些优化）
...
- Outlier Removal (and temporal clamping)
  ...
  - Outliers 在 rendering 领域有个名字叫 fireflies（火萤、萤火虫）。在图形领域，从理论上来讲并不应该去除它们，反而应该增大采样率来把 variance 降下去；但从 RTRT 的角度我们等不起
...
- Deferred Shading 延迟渲染
  ...
  - 延迟渲染的基本思路就是 2 pass rasterization：第一次不做 shading，只更新 depth buffer；第二次重新 rasterize，此时通过 depth test 的物体一定是最终可见的
    - 注：弹幕提醒这里似乎讲错了，这其实是 Early-Z 而不是 Deferred Shading，后者的重点在于 G-Buffer 和多光源
