...
- monocular HPS 有哪些困难？
  + 数据缺乏：缺乏大规模的 (image，mesh) 对。现存的这种数据基本是在实验室环境采集的，这样训练的模型在真实场景、野外场景的图片上泛化性不好
...
#q[
  ICON 作者按：ICON 现在支持 PyMAF, PARE 以及 PIXIE 三种 HPS，PARE 对遮挡好一些，PIXIE 手和脸准一些，PyMAF 最稳定，但依旧对一些很难的 case 束手无策。所以，尽管 *HPS 已经被人做烂了，围绕各种 corner case 一年能出几百篇论文*，但我们依然不能说这个问题解决了，也不能说，这个问题没有价值了，至少，对于 ICON 而言，HPS 的准确度，是一切的基础，HPS 挂了，迭代优化到猴年马月也没用
...
- *HMR*。参考 #link("https://zhuanlan.zhihu.com/p/441172308")[这篇解读]
  ...
  - HMR 如何解决前述困难？
    + 虽然缺乏成对的 (image，mesh) 数据，但是有大量的 2D keypoints 标注的图片数据集和大量单独的 3D mesh 动捕、动画数据。HMR 的一个关键贡献就是通过一个 GAN 框架充分利用这些 unpaired 的数据。具体来说，给定一张图片，网络推导出 3D mesh 的 $beta, th$ 参数和相机参数 (Generator)，然后将预测 3D mesh 投影到 2D 平面，让投影的 keypoints 和 2D keypoints 标注相匹配
...
- SMPLicit，没看，可以参考 #link("https://zhuanlan.zhihu.com/p/362132005")[CVPR 2021 | SMPL 人体模型有了，是不是还缺个衣服模型？SMPLicit 来了]
...
- 延伸阅读
  + #link("https://blog.csdn.net/m0_51661400/article/details/143334870")[3D 人体建模的前沿探索：细数主流模型与技术进展]
  + #link("https://blog.csdn.net/m0_51661400/article/details/143433865")[3D 人体建模的前沿探索（二）：深入解析SMPL-IK与多视角人体网格重建]