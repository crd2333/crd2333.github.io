...
- 先不管 RGB，PIFu 就是一个 “SHM image encoder + 预测 occupancy 的隐式函数” 训练出的端到端模型
  ...
  - $g(dot)$ 是一个 stacked hourglass model(SHM) image encoder
    - hourglass model 是一个长得像沙漏一样（看上面图）的对称网络，跟 U-Net 很像用到了 skip connection，经常用于人体姿势识别。主要 insight 是，传统卷积网络大多只使用最后一层特征，但人体姿态估计这种事情，往往需要多尺度的信息。而 stacked 就是把它们堆叠起来，复用全身关节信息来提高单个关节的识别精度
...
- 相比之下，ICON 从 SMPL body 那里得到一个更强的先验，然后交替优化，这个法向的效果看起来（毕竟我没跑过代码，笑）就有说服力得多