...
- *Semi-automatic stage* 半自动阶段
  ...
  - 在第一阶段的所有掩码上使用通用的 "object" 类别训练了一个 bounding box 检测器，自动检测置信度高的掩码。随后向标注员展示填充了可信掩码的图像，要求他们标注任何未标注的对象
  - 与 stage 1 一样，定期在新收集的数据上重新训练模型（$5$ 次）。随着模型改进，每个掩码（不包括自动掩码）的平均花费标注员的时间又升高（因为这些对象更难标注），每个图像的平均掩码数量增加（包括自动掩码）。总的来说，在这个阶段收集了 $180 K$ 张图像的 $10.2 M$ 个掩码
...
- SA-1B 数据集
  ...
  - *Masks*: Data Engine 产生了$1.1 B$ 个掩码，其中 $99.1%$ 是完全自动的，因此评估其质量至关重要。作者直接将它们与专业标注进行比较，主要结论是这些自动生成的掩码质量很高、属性很好：
    ...
    - Mask properties。主要对比了这些：
      + 对象分布 $->$ 大家都有摄影师偏见，SA-1B 已经是最小的那个
      + 每张图片的掩码数量 $->$ 图片多，每张图的掩码数量也最多
      + 图像相对掩码大小 $->$ 倾向于中小型掩码
      + 形状复杂性 $->$ 大家都差不多
