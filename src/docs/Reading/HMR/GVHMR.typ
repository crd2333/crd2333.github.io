...
- 一般的 human motion recovery 只需要 ${Ga^t_c, ta^t_c}^T_(t=0)$ 就可以了；但对 world-grounded setting 而言还需要 ${Ga^t_w, ta^t_w}^T_(t=0)$
  - 这个 world space $W$ 要求四平八稳、重力朝下（即 $y$ 轴朝下），可以注意到这并非良定义的，因为绕着重力轴的任意朝向都是合法的。本文把 $W$ 定义为 $GV_0$，即第零帧的 GV coordinate system
...
- 这样的 GV 坐标系有什么益处呢？
  ...
  - *用我自己的话来说（不保证正确）*，大概就是两点：第一，利用 GV 坐标系的定义降低网络学习任务的难度，降低了相机位姿和人体位姿之间的多义性（比如，预测出的相机偏一点、人也偏一点，这是合法的，但可能性太多了；预测相对 GV 坐标系的人体位姿则大大降低了不确定性的自由度），这样理解的话，不管是相机位姿的预测还是人体位姿的预测都更鲁棒了；第二，通过 GV 坐标系消除了 y 轴方向的累计误差
